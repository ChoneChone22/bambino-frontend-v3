...
export default function FranchiseSection() {
  return (
    <section
      id="franchise"
      className="py-24 md:py-32 bg-(--color-background) text-(--color-text)"
    >
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {benefits.map((benefit) => (
              <div
                key={benefit.title}
                className="p-6 border border-text-(--color-text) hover:border-black/30 transition-colors rounded-lg"
              >
                <benefit.icon className="w-8 h-8 mb-4 text-(--color-text)" />
